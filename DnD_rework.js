...
function transferBitches() {
  ...
  let costCp = cost.reduce((partialSum, a) => partialSum + a, 0);
  let transactionCp = transactionSum.reduce(
    (partialSum, a) => partialSum + a,
    0
  );
  if (costCp <= walletCp) {
    //if total user wallet in copper > total cost in copper
    for (let k = 0; k <= 4; k++) {
      //loops next while 4 times
      while (userWallet[i] > 0) {
        //subtract all coins in i of user wallet (i = 0 = copper, i = 1 = silver, i = 2 = electrum...)
        ...
        if (transactionCp >= costCp) {
          // converts sum of cost and sum of transaction and checks against each other
    ...
    //else not enough copper in wallet, too expensive
...
function removeBitches() {
  let change =
    transactionSum.reduce((partialSum, a) => partialSum + a, 0) -
    costSum.reduce((partialSum, a) => partialSum + a, 0); //take the transaction convert to copper, and subtract the sum converted to copper
  for (let i = 4; i >= 0; i--) {
...
console.log(
  "wallet in cp: " + walletSum.reduce((partialSum, a) => partialSum + a, 0)
);
console.log(
  "cost in cp: " + costSum.reduce((partialSum, a) => partialSum + a, 0)
);
...
console.log(
  "sum of cost: " + costSum.reduce((partialSum, a) => partialSum + a, 0)
);
